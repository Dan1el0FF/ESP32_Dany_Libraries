//Libreria extraida y actualizada de https://embeddedtutorials.com/eps32/esp32-wifi-cpp-esp-idf-station/
#include "wifi.h"
#include "esp_mac.h"
namespace WIFI
{
    ...
    esp_err_t Wifi::_get_mac(void)
    {
        ...
        esp_err_t status = esp_read_mac(mac_byte_buffer, ESP_MAC_WIFI_STA);

        if (status == ESP_OK)
        {
            snprintf(_mac_addr_cstr, sizeof(_mac_addr_cstr),
